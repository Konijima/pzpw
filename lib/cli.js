...
exports.add = async function(dirpath, ModID, ModName) {
    ...
    if (existsSync(`./src/${ModID}`))
        return console.log(colors.red(`Mod ${ModID} already exist in this project!`))
...
exports.switch = function(dirpath, branch) {

    if (!isValidWorkingDirectory())
        return console.log(colors.red(`Command must be executed at the root of your project directory.`));

    if (!["stable", "unstable"].includes(branch)) {
        return console.log(colors.bold("usage:"), colors.green(usages.switch));
    }

    try {
        console.log(colors.bold(`Reading package.json...`));
        let packageStr = readFileSync(`package.json`);
        let package = JSON.parse(packageStr);
        
        if (String(package.dependencies.PipeWrench).includes(`#${branch}`)) {
            console.log(colors.yellow(`Project is already on branch ${branch}`));
            return;
        }

        console.log(colors.bold(`Setting PipeWrench branch to ${branch}...`));
        package.dependencies.PipeWrench = `github:asledgehammer/PipeWrench#${branch}`

        console.log(colors.bold(`Writing package.json...`));
        writeFileSync(`package.json`, JSON.stringify(package, null, 4), "utf-8");

        console.log(colors.bold(`Updating node modules...`));
        sh.exec(`npm update`)
    }
    catch(error) {
        console.log(colors.red(`There was an error writing package.json`));
        console.log(error.message);
    }
