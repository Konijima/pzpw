...
exports.new = async function(dirpath, ModID, ModName, Author) {
    ...
    sh.mkdir(`./src/client/${ModID}`);
    sh.mkdir(`./src/server/${ModID}`);
    sh.mkdir(`./src/shared/${ModID}`);
...
helps.add = `Add a module to your project.`;
usages.add = `${config.name} add <ModuleName>`;
...
usages.compile = `${config.name} compile <dist|dev|export>`;
